...
from config import GEMINI_API_KEY, CRASH_PERIODS
...
def summarize_news(articles: List[Dict], category: str = "general") -> str:
    ...
    Falls back to extractive top-5 titles if Gemini unavailable.
    """
    if not articles:
        return "目前無可用的新聞資料。"
...
4. ⚠️ 需關注的風險因素（最多3項）
"""
    result = _gemini(prompt, max_tokens=600)
    if result:
        return result
...
def compare_with_crashes(
    ...
    crash_descriptions = {
        ...
        "2007-2009 金融海嘯":
            "次貸危機引發系統性金融風險，S&P500下跌約57%，失業率升至10%，信用市場凍結。",
        "2020 COVID-19 崩盤":
...
4. 📈 如果情況惡化，最可能的劇本
"""
    result = _gemini(prompt, max_tokens=800)
    if result:
        return result
...
4. ⚡ 觸發進一步降低風險的警示條件
"""
    result = _gemini(prompt, max_tokens=700)
    if result:
        return result
...
def analyze_overview(
    ...
    prompt = f"""
你是一位資深美股分析師，請根據以下即時市場數據，用**繁體中文**撰寫一份「市場綜合解讀」給一般投資人閱讀。
...
- 本系統衰退風險評分：{recession_score}/100（{recession_label}）

請提供：
1. **整體市場氛圍**（2-3句，直接說明現在市場是偏樂觀/中性/緊張，以及為什麼）
2. **三大關鍵訊號**（列出最重要的3個數據點，說明其意義與對股市的影響）
3. **未來一個月走勢判斷**（給出明確方向：偏多/震盪/偏空，並說明主要理由）
4. **一般投資人應注意什麼**（1-2個具體建議，避免使用過度技術性語言）

語氣要像跟朋友解釋，不要太學術，讓沒有財經背景的人也看得懂。
"""
    ...
    yc_signal = "倒掛（衰退警訊）" if yield_spread < 0 else "正常"
    return f"""**市場氛圍：{sentiment}**

目前 VIX={vix:.1f}，市場波動程度{sentiment}。殖利率曲線{yc_signal}（利差{yield_spread:+.2f}%）。
恐懼貪婪指數={fg_val}（{fg_cls}），衰退風險評分={recession_score}/100。
...
def analyze_stock(
    ...
    prompt = f"""
你是一位資深股票分析師，請根據以下數據對 {ticker}（{company_name}）做出**繁體中文**的綜合分析。
...
- 布林通道 %B：{bb_pct:.2f}（{'上軌附近' if bb_pct>0.8 else '下軌附近' if bb_pct<0.2 else '中間區間'}）
- ATR：{atr:.2f}（每日平均波動）
...
請提供：
1. **技術面判斷**（目前是多頭/空頭/盤整格局？關鍵支撐與壓力位在哪？）
2. **基本面評估**（估值是否合理？有沒有值得注意的風險？）
...
技術訊號：{signal} | RSI={rsi:.1f}（{rsi_txt}）| {'站上' if close>ma200 else '跌破'} MA200
...
def analyze_macro(
    ...
    prompt = f"""
你是一位宏觀經濟分析師，請根據以下美國總體經濟數據，用**繁體中文**為一般投資人撰寫綜合解讀。

【總體經濟指標】
- 失業率：{unemployment:.1f}%（月變動：{unemployment_chg:+.2f}%）
- CPI（消費者物價指數）：{cpi:.1f}（月變動：{cpi_chg:+.2f}）
- 聯邦基金利率：{fed_rate:.2f}%
...
4. **對股市的影響**（總體經濟環境對股市是順風還是逆風？哪些板塊相對有利？）
5. **與歷史的比較**（目前環境最像哪個歷史時期？投資人應從中學到什麼？）

請用淺白易懂的語言，讓沒有經濟學背景的人也能理解。
"""
    result = _gemini(prompt, max_tokens=1000)
    if result:
        return result
...
消費者信心={consumer_conf:.1f}，衰退風險評分={recession_score}/100。
...
def analyze_news_sentiment(
    ...
    prompt = f"""
你是一位市場情緒分析師，請根據以下新聞情緒數據，用**繁體中文**撰寫市場情緒綜合解讀。
...
請提供：
1. **市場情緒總結**（目前市場是恐慌/謹慎/中性/樂觀/貪婪？2-3句說明）
...
def analyze_mc_results(
    ...
    prompt = f"""
你是一位量化投資分析師，請根據以下蒙地卡羅模擬結果，用**繁體中文**為一般投資人解讀數字的實際意義。
...
- P01（最悲觀1%）：{p01:+.2f}%
- 虧損機率：{loss_prob:.1f}%
- 95% VaR：{var_95:+.2f}%（有5%機率損失超過此值）
- 95% CVaR：{cvar_95:+.2f}%（最差5%情境的平均損失）

【市場環境】VIX={vix:.1f}，殖利率利差={yield_spread:+.2f}%

請提供：
1. **用人話解釋P50**（中位數代表什麼？如果投資$18,605，最可能的結果是賺多少或虧多少？）
2. **風險評估**（VaR和CVaR代表什麼？最壞的情境有多糟？）
3. **模擬結果是否合理**（根據目前市場環境，這個預測偏樂觀還是偏保守？）
4. **投資決策建議**（根據這些數字，現在是好的進場時機嗎？有什麼需要特別注意的？）

請用具體金額舉例（假設投資$18,605），讓讀者更容易理解風險。
"""
    result = _gemini(prompt, max_tokens=900)
    if result:
        return result
    dollar_p50 = 18605 * p50 / 100
    return f"""**模擬結果快速解讀**

一個月後最可能的結果（P50）：{p50:+.2f}%，相當於 {'+' if dollar_p50>=0 else ''}\${dollar_p50:,.0f}。
虧損機率：{loss_prob:.1f}%，VaR(95%)={var_95:+.1f}%。
...
def analyze_portfolio(
    ...
    prompt = f"""
你是一位財務顧問，請根據以下投資組合模擬結果，用**繁體中文**為投資人提供個人化的綜合分析與建議。
...
請提供：
1. **投資組合健康度**（集中度是否過高？風險是否分散？2-3句直接評價）
2. **最大風險點**（哪一支股票或哪個配置最可能造成重大虧損？）
3. **避險效果評估**（避險配置是否真的有效降低風險？值不值得做？）
4. **具體改善建議**（如果要優化這個組合，最重要的1-2個調整是什麼？）
5. **心理準備**（用人話告訴投資人：最壞情況下可能損失多少，應該如何心理準備？）
...
"""
    ...
    dollar_p01 = total_value * p01 / 100
    return f"""**投資組合快速解讀**

最可能結果（P50）：{p50:+.2f}%（{'+' if dollar_p50>=0 else ''}\${dollar_p50:,.0f}）
最差1%情境（P01）：{p01:+.2f}%（\$-{abs(dollar_p01):,.0f}）
虧損機率：{loss_prob:.1f}%
